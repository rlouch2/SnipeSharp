...
namespace SnipeSharp.PowerShell.Cmdlets
{
    ...
    public class RemoveDepartment: PSCmdlet
    {
        /// <summary>
        ...
        /// <summary>
        /// <para type="description">A Department object.</para>
        /// </summary>
        [Parameter(
            Mandatory = true,
        ...
        protected override void ProcessRecord()
        {
            if(ParameterSetName == "ByName")
            {
                foreach(var name in Name)
                {
                    ...
                    {
                        var response = ApiHelper.Instance.DepartmentManager.Delete(item);
            ...
            } else
            {
                foreach(var item in Department)
                {
                    if(item == null)
                    {
                        WriteError(new ErrorRecord(null, $"Department cannot be null!", ErrorCategory.InvalidArgument, null));
                    } else if(ShouldProcess(item.Name ?? item.Id.ToString()))
                    {
                        var response = ApiHelper.Instance.DepartmentManager.Delete(item);
                        if(ShowResponse.IsPresent)
